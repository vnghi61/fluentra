...
Fluentra sử dụng một adapter chuẩn duy nhất là `OpenAICompatibleProvider` ([internal/platform/ai/provider_openai.go](file:///c:/Users/HP/Downloads/code/fluentra/internal/platform/ai/provider_openai.go)), tương thích với mọi dịch vụ cung cấp API chuẩn `POST /chat/completions` (OpenAI, Groq, Cerebras, OpenCode, OpenRouter, Mistral, Ollama, vLLM, LM Studio...).

Hệ thống quản lý AI qua **4 numbered provider slots** ([cmd/api/main.go:L155-L181](file:///c:/Users/HP/Downloads/code/fluentra/cmd/api/main.go#L155-L181)):
...
Trong [internal/platform/ai/module.go:L109-L113](file:///c:/Users/HP/Downloads/code/fluentra/internal/platform/ai/module.go#L109-L113):
```go
...
1. **[docs/development/phase-3-work-order-7.md:L125-L135](file:///c:/Users/HP/Downloads/code/fluentra/docs/development/phase-3-work-order-7.md#L125-L135)**:
...
2. **[.env.example:L177-L179](file:///c:/Users/HP/Downloads/code/fluentra/.env.example#L177-L179)**:
   > *"# Everything below describes internal/platform/ai's full specification... and is NOT read by any code yet."*
3. **[cmd/api/main.go:L155-L181](file:///c:/Users/HP/Downloads/code/fluentra/cmd/api/main.go#L155-L181)** và **[cmd/worker/main.go:L119-L145](file:///c:/Users/HP/Downloads/code/fluentra/cmd/worker/main.go#L119-L145)**: Struct `AI` chỉ nhận diện các trường `provider_1_*` đến `provider_4_*`. Thư viện `koanf` tự động drop tất cả các biến ngoài struct.
...
Theo thiết kế tại [docs/development/phase-3-work-order-12.md](file:///c:/Users/HP/Downloads/code/fluentra/docs/development/phase-3-work-order-12.md):
...
* **Production CI/CD:** Server Render (gói Free) không đủ tài nguyên để render TTS. Do đó audio được render qua GitHub Actions workflow:
  * File workflow: [.github/workflows/tts-render.yml](file:///c:/Users/HP/Downloads/code/fluentra/.github/workflows/tts-render.yml).
...
2. **Môi trường Production (CI/CD):**
   Vì GitHub Actions luôn tải file `.onnx.json` mới từ Hugging Face, cách xử lý triệt để nhất là thêm cờ `--length_scale` trực tiếp vào mã nguồn Go trong [internal/platform/media/piper.go:L76](file:///c:/Users/HP/Downloads/code/fluentra/internal/platform/media/piper.go#L76):
   ```go
   args := []string{
...
Tại [cmd/api/main.go:L374-L454](file:///c:/Users/HP/Downloads/code/fluentra/cmd/api/main.go#L374-L454):
...
Tại [internal/platform/job/cron.go](file:///c:/Users/HP/Downloads/code/fluentra/internal/platform/job/cron.go):
...
Tại [cmd/worker/main.go:L442-L447](file:///c:/Users/HP/Downloads/code/fluentra/cmd/worker/main.go#L442-L447):
```go
...
|---|---|
| [internal/platform/job/cron.go](file:///c:/Users/HP/Downloads/code/fluentra/internal/platform/job/cron.go) | Thêm quản lý `lastRun`, hàm `TriggerDue()` và `LastRun()` kiểm tra job quá hạn do server sleep. |
| [internal/platform/job/cron_trigger_test.go](file:///c:/Users/HP/Downloads/code/fluentra/internal/platform/job/cron_trigger_test.go) | Unit test kiểm tra `TriggerDue` và cơ chế debouncing chống gọi lặp. |
| [cmd/worker/main.go](file:///c:/Users/HP/Downloads/code/fluentra/cmd/worker/main.go) | Hook `cron.TriggerDue()` vào endpoint `/ready` để tự động bù job khi nhận ping từ API. |
| [cmd/api/main.go](file:///c:/Users/HP/Downloads/code/fluentra/cmd/api/main.go) | Kích hoạt `workerNudger.Nudge(ctx)` ngay khi API khởi động để đánh thức Worker. |
