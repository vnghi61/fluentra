...
3. **Vite API proxy vs CORS:**
   Vite proxy was chosen instead of CORS for the development environment. The Vite dev server is configured with `host: "0.0.0.0"` and a dev proxy on `/api` forwarding to `VITE_API_TARGET` (`http://api:8080` in Compose, `http://localhost:8080` on host). Proxying was selected over relying on CORS because it accurately mirrors the production Nginx reverse proxy topology (`/api` routed under same origin), prevents browser same-site cookie / credential restrictions during local development, and avoids configuration drift between development and production CORS settings. CORS support remains enabled on the Go API server for direct cross-origin client requests when needed.
