...
| **Do** | Schema `learn`: `review_cards`, `review_logs`, `srs_params`, `review_daily_stats`. Cards are created from `GradeResult.ReviewItems`, which WP8 already emits. Implement the due queue, the answer path (record a log, reschedule through the pure domain function), suspend, reset, and the daily stats rollup as a job. |
