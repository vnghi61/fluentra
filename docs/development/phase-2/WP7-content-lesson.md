...
| **Trap — two** | **(1)** Filtering in Go after selecting everything is the version of this bug that passes tests and leaks in production. Put `status = 'published'` in the SQL. **(2) Answer the open permission question before writing the guard.** `content.read.published` is in the catalogue (migration `1700000180`) and held by **admin only**. Six module `AGENT.md` files name it on learner reads, but `1700000020` gave the `user` role no named permissions and `TestAdminHoldsEverythingAndLearnerHoldsNothing` asserts that — so calling `Require(PermContentReadPublished)` here denies every learner. Pick one, record it in `content/DECISIONS.md`, and make the test agree: grant it to `user` and update that test, or drop the permission for learner reads and gate on authentication alone. Do not discover this at runtime. |
