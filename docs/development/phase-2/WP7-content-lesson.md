...
| **Trap — two** | **(1) The archive-mid-session question is half answered; finish it.** P7.2 settled the mechanics — archiving sets `content_items.status`, because the immutability trigger refuses to change a published version's status, so `content_versions.status` can never become `archived` (see `content/DECISIONS.md`). What is still yours: a learner who opened a lesson yesterday holds a **version id**, and `GetVersion` on it must keep working after the item is archived. The version stays readable; only *discovery* stops. Write the test that proves both halves. **(2) `media_refs` is `text[]` and carries no foreign key** — a Postgres array cannot. Nothing in the schema stops a version being published while it points at a `media_assets` row still `pending`, which is a learner opening a lesson whose audio does not exist. Publish resolves every `object_key` and refuses while any asset is not `ready`; prove it with a test. |
...
| **Trap — two** | **(1)** `lesson` depends on `content` and `cache` only. It must not import `learning` — the arrow goes the other way, and `UnlockChecker` is `learning`'s interface that `lesson` calls, per `learning/AGENT.md` §4. `go-arch-lint` will catch it; do not spend an hour discovering why. **(2) `activities.content_version_id` is a plain `uuid` column with no foreign key.** `activities` lives in `learn` and `content_versions` in `content`, and **DB4 permits exactly one cross-schema foreign key, `→ core.users(id)`**. Nothing in CI checks this — `go-arch-lint` reads imports, not migrations — so it is a review condition, and the same rule applies to every later table that points at a content version. |
...
| **Do** | Implement `GET /api/v1/courses`, `GET /api/v1/courses/{slug}`, `GET /api/v1/lessons/{id}` against the spec written in P7.1. Guard each with `Require(PermContentReadPublished)`, which the `user` role holds (migration `1700000180`) — **a signed-in learner reads, an administrator writes**. A learner sees published versions only, and the query enforces that rather than the handler filtering after the fact. Cache the course tree; invalidate on publish. |
...
| **Trap — two** | **(1) Filter on *both* statuses, in the SQL.** Filtering in Go after selecting everything is the version of this bug that passes tests and leaks in production — but so is filtering on the wrong column. P7.2 made archiving an **item**-level action: the immutability trigger refuses every update to a published version, so `content_versions.status` stays `'published'` for ever and a query filtered only on it **returns archived material**. Join `content_items` and require `content_items.status = 'published'` as well; `idx_content_items_status_kind` exists to serve exactly that. **(2)** The read/write split is settled and enforced, so do not re-derive it. `user` holds `content.read.published` and nothing else; `content.create`, `.edit`, `.review` and `.publish` are admin-only. `TestAdminHoldsEverythingAndLearnerReadsOnly` fails if a learner gains anything outside that one-item list, which is what stops a convenience grant during this task from quietly opening authoring to everyone. |
