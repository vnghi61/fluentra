...
| **Trap — two** | **(1)** Filtering in Go after selecting everything is the version of this bug that passes tests and leaks in production. Put `status = 'published'` in the SQL. **(2)** The read/write split is settled and enforced, so do not re-derive it. `user` holds `content.read.published` and nothing else; `content.create`, `.edit`, `.review` and `.publish` are admin-only. `TestAdminHoldsEverythingAndLearnerReadsOnly` fails if a learner gains anything outside that one-item list, which is what stops a convenience grant during this task from quietly opening authoring to everyone. |
