...
const adrs = [
  ...
  {
    n: 21, slug: "email-otp-challenges", title: "Email OTP challenges instead of verification links", tags: "security",
    context: "Registration must prove control of an email address. The conventional mechanism is a signed link. In our market the majority of learners open email on a phone, where a link launches a second browser context with no session — the original tab is abandoned, the registration is not completed, and the learner is left in an unverified state they do not understand. The same primitive is also needed for password reset and, later, for step-up verification on an untrusted device.",
    decision: "Build one generic challenge subsystem: `auth_challenges` rows carrying a `purpose` enum (`verify_email`, `login_otp`, `password_reset`, `link_oauth`), a 6-digit code stored as `HMAC-SHA256(code, server_key)`, a 10-minute TTL, a hard cap of 5 verification attempts, and single-use consumption. The client receives a `challenge_id`; the code goes to the email channel only. Resend has a 60-second cooldown and a cap of 3 issuances per subject per hour.",
    alts: [
      { name: "Signed verification links", pros: "One click; no typing; higher entropy in the token", cons: "Breaks the mobile flow by switching browser context; link scanners in corporate mail systems consume single-use tokens before the human clicks; harder to explain when it fails", why: "The context switch is not a minor annoyance — it is the dominant registration drop-off in mobile-first products. Link prefetching by mail scanners silently burning tokens is a second, harder-to-debug failure." },
      { name: "Magic-link login (no password at all)", pros: "No password to forget or leak", cons: "Every sign-in depends on email delivery latency; account recovery becomes email recovery; still has the context-switch problem", why: "It makes email an availability dependency on every single sign-in, not just on registration." },
      { name: "A separate table per purpose", pros: "Simpler individual schemas", cons: "Rate limiting, hashing, attempt capping, expiry and burning duplicated three times", why: "Three copies means three chances to get constant-time comparison or attempt capping wrong, and the bug would be silent in two of them." },
      { name: "SMS OTP", pros: "Familiar; independent of email", cons: "Per-message cost; SIM-swap risk; carrier delivery variance; requires collecting phone numbers we otherwise do not need", why: "More personal data, more cost, and a weaker channel than email for our threat model." },
    ],
    pos: ["The learner never leaves the tab they started in — the flow completes on one screen, on any device", "One implementation serves verification, password reset and future step-up, so the security-critical parts are written and tested once", "Mail scanners cannot consume a code by prefetching a URL", "A wrong code gives immediate, understandable feedback with a remaining-attempts count", "The code alone is useless without the `challenge_id`, which narrows the attack surface considerably"],
    neg: ["10^6 of entropy is far less than a signed token — the attempt cap and rate limiters are load-bearing, not decorative", "Two API round trips instead of one link click", "The learner must switch to their mail app and back, and type six digits", "A distributed guessing campaign across many challenges needs a global limiter to catch; the per-challenge cap alone is insufficient"],
    compliance: "Integration tests assert: single use, expiry, exactly 5 attempts then burn, constant-time comparison, cross-challenge code rejection, resend cooldown, and that the code appears in no response body, log record or span attribute. The global per-IP issuance limiter has its own test.",
    revisit: "If measured registration completion does not improve over the link-based baseline, or if email delivery latency makes the 10-minute window impractical for a significant share of learners.",
  },
  {
    n: 22, slug: "persistent-sessions", title: "Persistent sign-in: sliding idle window with an absolute cap", tags: "security",
    context: "Learners study in short daily sessions. Being asked to sign in again is friction at exactly the moment habit formation matters most, and it is the single most common complaint about consumer learning apps. The product requirement is 'sign in once and stay signed in'. The security requirement is that every credential has a bounded lifetime and a working revocation path. These are usually presented as opposites.",
    decision: "Refresh tokens become **sliding**: each rotation issues a replacement with a fresh idle window rather than inheriting the original expiry. Each session additionally carries an **absolute** expiry that activity does not extend. Idle windows are 30 days by default and 90 days on a device the learner explicitly chose to trust; the absolute cap is 180 days. Administrator accounts receive neither extension — 12-hour idle, 7-day absolute. Trusted devices are listed in the account and revocable from anywhere; a password change, reset or suspension revokes all of them.",
    alts: [
      { name: "A long-lived or non-expiring 'remember me' token", pros: "Trivially satisfies the requirement", cons: "An immortal credential; a stolen cookie is valid forever; no natural point at which possession is re-proven", why: "It cannot be reasoned about. 'When does this stop being dangerous?' has no answer, and that is disqualifying for a credential." },
      { name: "Fixed 30-day expiry, no sliding", pros: "Simplest to reason about", cons: "A learner active every single day is still signed out on day 31, for no security benefit — their possession was re-proven daily", why: "It imposes the cost of expiry without the corresponding benefit; expiry should mean 'we have not seen you in a while', not 'time passed'." },
      { name: "Sliding window with no absolute cap", pros: "The learner truly never signs in again", cons: "A stolen token that is used regularly renews itself indefinitely — the theft becomes permanent and invisible", why: "The absolute cap is precisely what bounds a successful theft. Without it, sliding is the immortal-token option wearing a different name." },
    ],
    pos: ["An actively studying learner never sees a login screen again, which is the actual product requirement", "Every credential still has two independent bounds: inactivity and absolute age", "A stolen refresh cookie expires at the absolute cap even if the attacker keeps it alive", "Trusted devices are visible and individually revocable, so the learner can act on a lost phone", "Reuse detection from ADR-0007 continues to apply unchanged, so theft remains *detectable* as well as bounded", "Admin accounts are explicitly excluded, so the highest-privilege sessions stay short"],
    neg: ["A longer idle window is a longer exposure window for a stolen cookie", "More state per session: idle window, absolute expiry, device linkage", "A learner who clears browser storage looks like a new device and must sign in again", "The interaction between sliding rotation and the absolute cap needs explicit tests, or one of the two bounds silently stops applying"],
    ...
    revisit: "If session-theft incidents occur, or if a compliance requirement imposes a shorter maximum. The three numbers (30/90/180 days) are configuration and can be tightened without a code change.",
  },
  {
    n: 23, slug: "google-oauth-linking", title: "Google OAuth and the account-linking policy", tags: "security",
    context: "Social sign-in materially reduces registration friction, and Google covers the overwhelming majority of our target market. The difficulty is not the protocol; it is deciding what happens when a Google identity presents an email address that already exists locally. Getting that decision wrong is a well-known account-takeover path that appears in production systems regularly.",
    decision: "Implement Google sign-in using the authorization code flow with PKCE, a server-side single-use `state`, and a `nonce` verified in the returned ID token, which is validated against Google's cached JWKS for signature, `iss`, `aud` and `exp`. Linking policy: link automatically **only** when Google asserts `email_verified: true` **and** the address matches an already-verified local account. If the matching local account is unverified, refuse with `OAUTH_ACCOUNT_CONFLICT` and require an OTP challenge on that address first. If no local account exists, create one already verified. Unlinking is refused when it would leave the account with no sign-in method.",
    alts: [
      { name: "Auto-link on any email match", pros: "Smoothest possible experience", cons: "An attacker registers victim@example.com locally without verifying it, waits, and later signs in through Google to claim the account — or the reverse, depending on which side is created first", why: "This is the takeover path. The convenience is real but it is bought with the account's integrity." },
      { name: "Never link — separate accounts per provider", pros: "No linking logic at all; no takeover path", cons: "A learner who registered with a password and later taps 'Continue with Google' silently gets a second, empty account and loses their progress", why: "It converts a security problem into a support problem, and the support problem is the one learners actually experience." },
      { name: "Implicit flow, or skipping PKCE as a confidential client", pros: "Slightly less code", cons: "Implicit is deprecated; without PKCE an intercepted authorization code on a mobile browser is exchangeable", why: "PKCE costs almost nothing to add and closes a real interception path on exactly the devices most of our learners use." },
      { name: "Include Apple sign-in now", pros: "Covers iOS learners; required by the App Store when other social logins exist", cons: "Additional integration with different token semantics and a hide-my-email relay address, for no web benefit today", why: "Deferred to whenever an iOS app is actually built, at which point it becomes a requirement rather than a nice-to-have." },
    ],
    pos: ["One tap to register on mobile, where typing a password is the highest-friction step", "Email verification is inherited from Google, so those accounts skip the OTP step entirely", "The takeover path is closed explicitly, and the refusal is a documented, testable branch rather than an accident", "State, nonce and PKCE together make CSRF and code interception concrete, tested defences rather than assumptions", "Every failure branch raises a security event, so an attempted takeover is visible in the security dashboard"],
    neg: ["A dependency on Google's availability for those learners' sign-in", "The `OAUTH_ACCOUNT_CONFLICT` branch requires clear UI copy or it reads as a bug to the learner", "More surface to test: five distinct linking outcomes, each with its own correct behaviour", "Learners who lose access to their Google account need a recovery path that does not go through Google"],
    compliance: "Integration tests cover all five linking outcomes plus forged, reused and expired `state`, and ID tokens failing each of signature, `iss`, `aud`, `exp` and `nonce`. A test asserts that no partial account is created on any failure branch.",
    revisit: "When an iOS application ships — Apple sign-in becomes an App Store requirement at that point, and the linking policy must be extended to its relay addresses.",
  },
  {
    n: 24, slug: "mobile-first-responsive", title: "Mobile-first responsive UI as a baseline requirement", tags: "frontend",
    context: "Language learning is a habit built in short sessions, and those sessions happen on a phone — on a commute, in a queue, before bed. The original frontend specification treated responsiveness as an implicit quality attribute, which in practice means it is verified late, by hand, and inconsistently. Meanwhile several core interactions — recording speech, drilling review cards, typing an essay — have genuinely different ergonomics on a touch device.",
    decision: "Adopt mobile-first as the development default rather than an adaptation: every component is built at the 375 px viewport first and enhanced upward. Enforce a concrete baseline — minimum 44×44 CSS px touch targets, 16 px minimum input font size, safe-area insets honoured, no hover-only interaction, virtual-keyboard-aware layout via `visualViewport`, bottom navigation on small viewports and a sidebar above `md`. Playwright runs every E2E journey across four device projects (iPhone-class, Android-class, tablet, desktop), and the performance budget is stated for a mid-tier Android device on 4G, not for a developer laptop.",
    alts: [
      { name: "Desktop-first, adapted down", pros: "Matches how the team develops day to day", cons: "Mobile becomes a series of overrides and exceptions; touch ergonomics are discovered late; the layouts that break are the ones most learners see", why: "It optimises for the environment the developers use rather than the one the learners use." },
      { name: "A separate mobile web application", pros: "Each surface fully optimised", cons: "Two codebases, two release cycles, two sets of bugs, for one product", why: "Unjustifiable at our team size, and the divergence starts immediately." },
      { name: "Build a native mobile app now", pros: "Best possible mobile experience; push notifications; offline", cons: "A third codebase, app store review cycles, and a second API consumer before the web product has validated retention", why: "Explicitly a year-one non-goal. A responsive web app is how we learn whether the product retains at all." },
      { name: "Responsive by convention, verified manually", pros: "No tooling cost", cons: "Unenforced conventions decay; regressions are found by users", why: "The same reasoning as every other rule in this repository — if CI does not check it, it is a suggestion." },
    ],
    pos: ["The primary usage context is the default development context, so mobile breakage is caught immediately", "Touch-target and font-size rules eliminate the two most common mobile usability defects before review", "Device-matrix E2E turns responsiveness from an opinion into a pass/fail signal", "A realistic performance budget prevents shipping something that only feels fast on a developer machine", "The same discipline prepares the UI for a future native shell without committing to one"],
    neg: ["Every component costs slightly more to build, because two layouts are considered from the start", "The E2E matrix multiplies runtime — mitigated by sharding and by running the full matrix nightly rather than on every push", "Some desktop-oriented admin screens gain little from the mobile constraint and still pay its cost"],
    compliance: "`eslint` rules and design-system primitives enforce touch-target and font-size minimums. Playwright device projects run in CI. The bundle and Web Vitals budgets are checked per pull request. A component merged without a mobile story fails review.",
    revisit: "If a native mobile application is built, at which point the web app's responsibility for small viewports may narrow — but not before retention on web is demonstrated.",
  },
];
