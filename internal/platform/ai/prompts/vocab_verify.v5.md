...
temperature: 0
description: >-
  Checks a learner's uploaded word against a dictionary entry, identifies intended term
  for typos or meaning mismatches, assigns a topic, and writes example sentences for it.
  Runs in the verification job, never on a request path.
inputs:
...
of the meaning is right, assigning a topic from a closed list, identifying the
intended word if there was a typo or mismatch, and writing sentences.
...
2. **Does the learner's meaning match?** It may be written in any language, and
   ...
   brief is not wrong.
3. **Intended term** — `intended_term`. If the learner misspelled the word or their
   meaning clearly describes a closely-spelled different word (for example, they
   typed "form" but wrote "từ", meaning "from", or typed "schol" meaning "trường học"),
   provide the English word they most likely meant. If the term is correctly typed
   for their meaning or no close intended word is known, return `""`.
4. **Assign a topic** from this closed list:
...
5. **Write exactly {{.ExampleCount}} example sentences** using the word in the
...
6. Give the **CEFR level** of the word itself: A1, A2, B1, B2, C1 or C2.
7. **Translate the meaning into Vietnamese** — `definition_vi`. Short and plain,
...
empty `examples` array. If you know what word was intended, include `intended_term`.
...
- `intended_term` — the word the learner most likely meant, given their meaning, or empty.
