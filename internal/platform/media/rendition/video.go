...
func renderPoster(ctx context.Context, ffmpegBin string, req RenderRequest) (*RenderResult, error) {
	...
	if _, err := cmd.CombinedOutput(); err != nil {
		...
		if errRetry != nil {
			return nil, fmt.Errorf("ffmpeg poster extraction failed: %w (output: %s)", errRetry, strings.TrimSpace(string(outBytesRetry)))
		}
	}
...
func renderVideoProfile(ctx context.Context, ffmpegBin string, req RenderRequest, maxH int) (*RenderResult, error) {
	...
	// ffmpeg -nostdin -protocol_whitelist file -i <src> -vf scale=-2:'min(maxH,ih)' -c:v libx264 -preset veryfast -crf 23 -c:a aac -b:a 128k -movflags +faststart -y <out>
	...
	if err != nil {
		return nil, fmt.Errorf("ffmpeg video transcode (%dp) failed: %w (output: %s)", maxH, err, strings.TrimSpace(outputStr))
	}
